...
public class EntityMountWrapper implements PacketWrapper {
    private static final Entity fakeNmsEntity = new ArmorStand(net.minecraft.world.entity.EntityTypes.ARMOR_STAND, MinecraftServer.getServer().overworld());
    ...
    public Object toNativePacket() {
        List<Entity> passengers = Arrays.stream(passengerIds).mapToObj(id -> {
            Entity passenger = new ArmorStand(net.minecraft.world.entity.EntityTypes.ARMOR_STAND, MinecraftServer.getServer().overworld());
            passenger.setId(id);
            return passenger;
        }).toList();
